...
import static com.hdrescuer.hdrescuer.common.Constants.BASE_URL_MANAGER;
...
public interface AuthApiService {
        // Se necesita una clase para parsear la respuesta JSON
    class NodeIpResponse {
        public String ip; // Debe coincidir con la clave JSON "ip"
    }
     ...
     * @return Call
     */
//     @POST("http://192.168.1.158:8000/api/datarecovery/data")
    @POST
    Call<String> setUserData(@Url String url, @Body RequestSendData userData);

    /**
     * Método que realiza llamada al servidor para obtener la IP del nodo disponible
     * @author Jose Manuel
     * @return Call
     */
    @GET(BASE_URL_MANAGER + "/api/available-node")
    Call<NodeIpResponse> getAvailableNodeIp();

    /**
     * Método que realiza llamada al servidor para comprobar la salud del nodo
     * @author Jose Manuel
     * @param nodeHealthUrl
     * @return
     */
    @GET
    Call<Void> checkNodeHealth(@Url String nodeHealthUrl);
